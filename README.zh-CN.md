...
Codex 额度只有在本地服务商记录包含 `rate_limits` 时才会显示。缺少这个字段不会影响
用量和成本统计。
...
- Claude Code 和 Codex 的会话存储只会被读取。
- 提示词、工具参数、服务商地址、鉴权头和凭据不会进入浏览器载荷或日志。
- 正常启动会短暂保留临时报告文件，随后自动删除。
- 跨运行保留的额度缓存包含额度状态和时间信息，不包含 OAuth 令牌。

运行时有三条明确记录的网络路径。CodeBurn 使用默认 USD 显示币种时，
Frankfurter 请求不会启用。
...
OAuth 令牌始终只读，不会进入快照、日志或错误信息。额度请求失败时，本地面板仍会显示。

## 工作方式

1. 隐藏启动器运行 `Generate-ClaudeReport.ps1`。
2. CodeBurn 和 ccusage 汇总本地 Claude 与 Codex 元数据。
3. 生成器核对模型成本，加入真实的 Claude 额度，再写出最小化的临时快照。
